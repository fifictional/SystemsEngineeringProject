...
/*********** Motor Scaling (NEW) ***********/
const float U_TO_SPEED = 150.0;   // try 80..300
...
/*********** PID Instance (matches your demo: Kp=55, Ki=0.001, Kd=1.5, N=10) ***********/
PID_controller pid(55.0f, 0.001f, 1.5f, 10.0f);
...
void loop() {
  ...
  if (now - lastTime >= (unsigned long)(dt * 1000.0f)) {
    lastTime = now;
    ...
    theta = pCnt / PEND_COUNTS_PER_RAD;  // rad
    x     = cCnt / CART_COUNTS_PER_M;    // m

    // velocities (telemetry only for now)
