#include <Wire.h>
#include <Motoron.h>

// Motoron Controller
MotoronI2C mc1(0x10);

// LQR Gain Matrix
const float K[4] = {
  -8.23606798,
  -5.81322011,
  55.89839879,
  12.36709747
};

// Swing-Up Parameters
const float TARGET_ENERGY = 1.0;

// Switch Angle from Swing-Up to LQR
const float SWITCH_ANGLE = 0.3;

// Timing
const float dt = 0.002;   // 500 Hz
unsigned long lastTime = 0;

// Encoder Pins
const int PEND_A = 3;
const int PEND_B = 5;
const int CART_A = 2;
const int CART_B = 4;

// Encoder Counts
volatile long pendCount = 0;
volatile long cartCount = 0;

volatile int pendLastA = 0;
volatile int cartLastA = 0;

// State Variables
float theta = 0, theta_dot = 0;
float x = 0, x_dot = 0;

float theta_prev = 0;
float x_prev = 0;

// Scaling Constants
const float PEND_COUNTS_PER_RAD = 310.7;
const float CART_COUNTS_PER_M = 2667.0;

// Safety Limits
const float THETA_LIMIT = 1.2; // rad
const float X_LIMIT = 3;       // m

// Motor Scaling
const float U_TO_SPEED = 1000.0;
const int MAX_MOTOR_SPEED = 1000;

// Deadzone Compensation
const int DEADZONE_FWD = 15;
const int DEADZONE_REV = 15;

// System Status
bool systemActive = false;

// Pendulum Encoder Interrupt
void pendISR() {
  int A = digitalRead(PEND_A);
  int B = digitalRead(PEND_B);
  if (A != pendLastA) {
    if (B != A) pendCount++;
    else pendCount--;
    pendLastA = A;
  }
}

// Cart Encoder Interrupt
void cartISR() {
  int A = digitalRead(CART_A);
  int B = digitalRead(CART_B);
  if (A != cartLastA) {
    if (B != A) cartCount++;
    else cartCount--;
    cartLastA = A;
  }
}

// Stop Motors
void stopMotors() {
  mc1.setSpeed(2, 0);
  mc1.setSpeed(3, 0);
}

// Safety Check
bool checkSafety() {
  if (abs(theta) > THETA_LIMIT) return false;
  if (abs(x) > X_LIMIT) return false;
  return true;
}

// Drive Motors with Deadzone and Saturation
void drive(float u) {
  int speed = (int)(u * U_TO_SPEED);

  if (speed > 0) speed += DEADZONE_FWD;
  else if (speed < 0) speed -= DEADZONE_REV;

  speed = constrain(speed, -MAX_MOTOR_SPEED, MAX_MOTOR_SPEED);

  mc1.setSpeed(2, speed);
  mc1.setSpeed(3, speed);
}

// Setup Function
void setup() {
  Serial.begin(115200);
  Wire.begin();

  mc1.setBus(&Wire);
  mc1.reinitialize();
  mc1.disableCrc();
  mc1.disableCommandTimeout();
  mc1.clearResetFlag();

  mc1.setPwmMode(2, 1);
  mc1.setPwmMode(3, 1);

  pinMode(PEND_A, INPUT_PULLUP);
  pinMode(PEND_B, INPUT_PULLUP);
  pinMode(CART_A, INPUT_PULLUP);
  pinMode(CART_B, INPUT_PULLUP);

  pendLastA = digitalRead(PEND_A);
  cartLastA = digitalRead(CART_A);

  attachInterrupt(digitalPinToInterrupt(PEND_A), pendISR, CHANGE);
  attachInterrupt(digitalPinToInterrupt(CART_A), cartISR, CHANGE);

  Serial.println("Bang-Bang Swing-Up Start...");
  delay(2000);

  noInterrupts();
  pendCount = 0;
  cartCount = 0;
  interrupts();

  theta = 0; x = 0;
  theta_dot = 0; x_dot = 0;
  theta_prev = 0; x_prev = 0;

  systemActive = true;
  lastTime = micros();
}

// Main Loop
void loop() {

  if (!systemActive) {
    stopMotors();
    return;
  }

  if (micros() - lastTime < (unsigned long)(dt * 1000000)) return;
  lastTime = micros();

  noInterrupts();
  long pCnt = pendCount;
  long cCnt = cartCount;
  interrupts();

  // Convert Encoder Counts to Physical Units
  theta = pCnt / PEND_COUNTS_PER_RAD;
  x     = cCnt / CART_COUNTS_PER_M;

  // Compute Derivatives
  theta_dot = (theta - theta_prev) / dt;
  x_dot     = (x     - x_prev)     / dt;

  theta_prev = theta;
  x_prev     = x;

  // Safety Check
  if (!checkSafety()) {
    systemActive = false;
    stopMotors();
    return;
  }

  // Control Law
  float u;

  if (abs(theta) < SWITCH_ANGLE) {
    // LQR Control
    u = K[0] * x +
        K[1] * x_dot +
        K[2] * theta +
        K[3] * theta_dot;
  } else {
    // Bang-Bang Swing-Up
    float E = 0.5 * theta_dot * theta_dot + (1 - cos(theta));
    float dE = TARGET_ENERGY - E;

    float dir = (theta_dot > 0) ? 1.0 : -1.0;

    if (dE > 0) u = 8.0 * dir;
    else         u = -3.0 * dir;

    // Prevent cart from moving too far
    if (abs(x) > 0.4) u += 4.0 * x;

    // Minimum drive to overcome static friction
    if (u > 0) u = max(u, 2.0);
    else       u = min(u, -2.0);
  }

  // Apply Motor Command
  drive(u);

  // Debug Output
  static int pc = 0;
  if (pc++ > 80) {
    Serial.print(theta);     Serial.print(" ");
    Serial.print(theta_dot); Serial.print(" ");
    Serial.print(x);         Serial.print(" ");
    Serial.println(u);
    pc = 0;
  }
}
